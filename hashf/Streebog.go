...
func StreebogHash(data []byte, size int) []byte {
    var hsh [BlockSize]byte
    var chk [BlockSize]byte
    var n uint64
    ...
    for len(buf) >= BlockSize {
        ...
        copy(block[:], buf[:BlockSize]) //64
        hsh = g(n, hsh, block) // обрабатываем блок
        chk = add512bit(chk, block) // обновляем контрольную сумму
        n += BlockSize * 8 // увеличиваем счетчик обработанных битов
        buf = buf[BlockSize:]
    }

    var block [BlockSize]byte
    copy(block[:], buf)
    block[len(buf)] = 1 // добавляем 1 в конец данных
    hsh = g(n, hsh, block) // обрабатываем последний блок


    var lengthBlock [BlockSize]byte
    binary.LittleEndian.PutUint64(lengthBlock[:], n+uint64(len(buf))*8) // добавляем длину сообщения в битах // LittleEndian - порядок байт, в котором младший байт идет первым
    hsh = g(0, hsh, lengthBlock)


    hsh = g(0, hsh, add512bit(chk, block)) // добавляем контрольную сумму
...
func add512bit(chk, data [BlockSize]byte) [BlockSize]byte {
    ...
    for i := 0; i < BlockSize; i++ {
        carry = uint16(chk[i]) + uint16(data[i]) + (carry >> 8) // скаладываются 2 байта и добавляется перенос от предыдущей опреации
        result[i] = byte(carry & 0xFF) // оставляем только младшие 8 бит
    }
    return result
}
...
func g(n uint64, hsh, data [BlockSize]byte) [BlockSize]byte {
    ...
    for i := 0; i < 8; i++ {
        r[i] = hsh[i] ^ ns[i] // XOR первых 8 байт хеша и счетчика
    }
    copy(r[8:], hsh[8:])
...
func l(data [BlockSize]byte) [BlockSize]byte {
    ...
    for i := 0; i < 8; i++ {
        ...
        binary.LittleEndian.PutUint64(result[i*8:(i+1)*8], res64)
    }
    return result
}