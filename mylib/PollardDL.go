...
func NewF(c [2]*big.Int, a, b, p *big.Int) [2]*big.Int {
	cN := new(big.Int).Mul(FastPowMod(a, c[0], p), FastPowMod(b, c[1], p))
	cN.Mod(cN, p)
...
func PollardDL(u, v, a, b, p *big.Int) *big.Int {
	...
	for i := 0; i < 1000; i++ {
		...
		cN := new(big.Int).Mul(FastPow(a, c[0]), FastPow(b, c[1]))
		cN.Mod(cN, p)

		dN := new(big.Int).Mul(FastPow(a, d[0]), FastPow(b, d[1]))
		dN.Mod(dN, p)
		...
		if cN.Cmp(dN) == 0 {
			...
			solutions := SloveFirstDegreeComparsion(aC, bC, new(big.Int).Sub(p, big.NewInt(1)))
			return solutions
		}
	}
