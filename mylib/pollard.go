...
func PollardRho(n *big.Int) *big.Int {
	...
	for i := 1; i < 1001; i++ {
		...
		d, _, _ := ExtendedGCD(sub.Abs(sub), n)
...
func PollardRhoMinusOne(n *big.Int) *big.Int {
	...
	for _, p := range primes {
		...
		exp := FastPowMod(pBig, big.NewInt(int64(l)), n) 
		a = FastPowMod(a, exp, n)                        // a = a^(p^l) mod n
		d, _, _ = ExtendedGCD(new(big.Int).Sub(a, big.NewInt(1)), n)
		if d.Cmp(big.NewInt(1)) != 0 && d.Cmp(n) != 0 && d.Cmp(n) == -1 {
			return d
		}
